...
def open_exe(st1):
    ...
    pyautogui.press('win')
    wait()
    pyautogui.write('App:Superdigital', interval=0.50)
    pyautogui.press("enter")
    wait()
    print("Abrir programa finalizado")
    st1 = True
    return st1
...
def action_pay(st2):
    wait()
    pyautogui.moveTo(x=1089, y=854)
    wait()
    pyautogui.click()
    wait()
    pyautogui.moveTo(x=969, y=851)
    wait()
    pyautogui.write("576677")
    wait()
    print("search button pay")
    pyautogui.moveTo(x=1083, y=980)
    print("wait...",  wait())
    print("click on target")
    pyautogui.click()
    
    print("search btn not")
    pyautogui.moveTo(x=983, y=630)
    print("wait..", wait())
    print("click on target ")
    pyautogui.click()
    
    print("search field")
    pyautogui.moveTo(x=856, y=286)
    print("wait...", wait())
    print("click on target")
    pyautogui.click()
    wait()
    print("write the code")
    pyautogui.write(read_pdf.codigo[0])
    st2 = True
    return st2
...
def process():
    ...
    if(open_exe(st1)):
        if(action_pay(st2)):
            print("Fazendo a ação de pagar")

    else:
            print("verificar a etapa de abrir programa")
